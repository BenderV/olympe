...
@Injectable()
export class OpenAITrainedModel extends OpenAIModel {
  modelName = null; // No default model name
  // acceptedModels = ['davinci:ft-personal-2022-07-18-21-06-04']

  // async call(modelName, prompt) {
  //   // TODO: fix add query in params
  //   const response = await this.openai.createCompletionFromModel({
  //     model: modelName,
  //     prompt,
  //   });
  ...
  //   query: string,
  // ): Promise<string> {
  //   const tables = await this.tablesService.getDatabaseSchema(databaseId);
  //   const schemaYaml = yaml.dump(tables);

  //   const promptTemplate = fs.readFileSync(
  //     path.resolve(__dirname, './openai.trained.ejs'),
  //     {
  //       encoding: 'utf8',
  //     },
  //   );

  //   // TODO: remove HTML encoding " -> &#34;
  //   const prompt = ejs.render(promptTemplate, {
  //     schemaYaml,
  //     query,
  //   });
  ...
  async prepareTrainingSets() {
    const queries = await this.queriesService.getAllValidatedQueries();
    const trainingSet: object[] = [];
    ...
    await Promise.all(
      queries.map(async (query) => {
        try {
          ...
          // ' ' before completion and '---' stop sign
          const completion = ` ${query.validatedSQL}\n---`;
    ...
    //   }
    // }
    // write JSONL lines file
    const output = trainingSet.map((item) => JSON.stringify(item)).join('\n');
    this.logger.log(`writing training set (${trainingSet.length} examples)`);
    fs.writeFileSync('../model/training_set.jsonl', output);
  }
}
